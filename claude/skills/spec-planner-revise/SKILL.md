...
spec-planner-plan の「フェーズ 1」と**完全に同一**（`TeamCreate`/`SendMessage`/`TeamDelete` を呼ばない、戻り値 200 tokens 以内、全文 Read 禁止、critic は `critic-findings.md` 追記のみ、`model` 指定の振り分けルールも共通）。本スキルでの `model` 割当は:
...
1. **モード明示**: 「これは新規設計ではなく**既存設計の改訂**。既存の判断と根拠を尊重し、変更する場合は `revision-history.md` に理由を残す」
2. **変更マーカー禁止**: 元ファイルに `v2` / `rev2` / `（改訂）` 等のマーカーを残さない。設計ドキュメントは常に「最新の確定版」として書く。履歴は `revision-history.md` のみに集約
3. **minutes.md 不可侵**: `minutes.md` は初回設計時の議事録。本スキルでは Read も不要（Write/Edit 禁止）
4. **改訂履歴の書き方**: `revision-history.md` の末尾に `## Revision R{N} (<YYYY-MM-DD>)` 見出しで追記。論点→結論の形式。逐語議論は書かない
5. **スコープ逸脱の扱い**: 修正指示と無関係な改善は、その場で適用せず `open-issues.md` に提案として積む
6. **critic 専用**: 「特に**回帰観点**を重視する。この変更で壊れる既存要求はないか、`open-issues.md` との矛盾はないか、過去の `revision-history.md` で記録された判断と衝突しないかを明示的に洗う」
...
### ステップ 1: analyst（単発・影響範囲特定）

- 目的: 修正指示を要求分解し、影響範囲（どのファイルのどのセクション）を特定。`requirements-mapping.md` に新規/変更要求の対応行を追記
- prompt 固有事項: 修正指示本文 + 0-3 で拾った冒頭抜粋 + `open-issues.md` 全文
- 戻り値後: `task-state.md` の `Next Action` に「改訂対象ファイル: design.md §X / data-model.md §Y」等を記録

### ステップ 2: 該当ファイル改訂（単発・必要なものだけ）

影響範囲に応じて、以下のうち**必要な subagent だけ**単発呼び出しする（並列化しない、逐次で呼ぶ）:

- `design.md` / `open-issues.md` の改訂 → architect
- `data-model.md` / `usecases.md` の改訂 → modeler
- 両方必要なら architect → modeler の順で逐次呼び出し（architect の更新が modeler の前提になりがちなため）

prompt 固有事項: 「対象セクションだけを Edit。章構成の大幅再編は不可。変更マーカー禁止。変更箇所の概要を戻り値 summary に 1 行で記載」

### ステップ 3: 軽量 critic（単発・1 往復）
...
### ステップ 4: 最終 critic（単発・回帰観点厚め）

- 目的: 改訂完了後の最終確認。既存成果物全体と改訂差分の整合を厚めに見る
- prompt 固有事項: 「改訂箇所を中心に、要求網羅・整合性・回帰リスク・非機能・過去 Revision との整合を一通り確認。severity タグ必須」
- 書き込み: `critic-findings.md` の `## Round (revise-R{N}, final)` セクション

### ステップ 4.5: ドメイン専門家チェック（条件付き・単発）
...
### ステップ 6: scribe（単発・最終統合、改訂版）

blocker ゼロ確定後に 1 回だけ起動する。prompt には以下を順に指示:

1. **整合チェック**: 改訂箇所と既存記述の境目で、用語・ID・テーブル名・参照切れ・改訂マーカー混入（`v2` / `rev2` / `（改訂）` 等）が無いか確認。不整合は直接 Edit で直す
...
- **ファイル全文 Read 禁止**: メインも subagent も、必要なセクションだけ offset/limit で Read。全文通読が必要なのは scribe の整合チェック時のみ
