...
4. **実装** — *実装ルール*に従う。UI タスクの場合は *UI サブフェーズ* (4a〜4g) を実行
...
- **デフォルトブランチ上 (`master` / `main` / `develop`)** → デフォルトブランチへの直接コミットを避け、wt で機能ブランチを自動作成してからそちらでコミットする。手順:
  ...
  2. `wt add -b` は別ディレクトリに新規 worktree を切るため、現在の作業ツリーにある未コミット変更を退避して持ち越す:
     ```bash
     git stash push -u -m dev-task-autobranch
     wt -s add -b <branch> <default-branch>   # 別ディレクトリに worktree+ブランチ作成 (-s で tmux 切替を抑止)
     wt_path=$(git worktree list --porcelain | awk -v ref="refs/heads/<branch>" '
       /^worktree / { p = $2 } $0 == "branch " ref { print p }')
     cd "$wt_path"
     git stash pop
     ```
  3. 以降の commit/push はこの worktree 内で行う。
  - worktree は**必ず wt 経由**で作る (作成先を自己判断せず、`git worktree add` を直接叩かない)。
