---
name: dev-task
description: Linear チケット ID または自然言語で指定された開発タスクを、最小差分かつ高品質に実装する。Kotlin、React TypeScript、protobuf、Node.js を主対象に、既存パターン調査、実装、型/ビルド/テスト/lint、レビュー、commit/push まで進める。「ABC-123 を実装して」「このチケットお願い」「このバグ直して」「〜の対応を追加して」「動くようにして」「このデザインを実装して」「Figma から作って」「このコンポーネント作って」「ピクセルパーフェクトに」など、リポジトリ内のコード変更を伴う依頼で使う。Figma 起点 UI タスクでは Figma MCP、Storybook、Playwright で視覚検証する。承認済みプランの実装開始時も使う。
---
...
- **承認済みプラン** (plan mode 承認後や「Implement the following plan」等) → *承認済みプランからのエントリ* に従いフェーズ 4 から開始
...
最初に `references/execution-actors.md` を読み、実行主体 (Claude Code / Codex) ごとの tool、プラン承認、レビュー、Skill ディレクトリ解決方法を確認する。
...
3.5. **プラン作成 (PLAN_REQUIRED のみ)** — 実行主体ごとの承認経路でプランを提示する
4. **実装** — *実装ルール*に従う。担当宣言と委譲可否は `references/execution-actors.md` に従う。UI タスクはサブフェーズ 4a〜4g を実行
5. **検証** — 型/ビルド/テスト/lint。失敗したら修正して再実行
6. **レビュー (trivial 除く)** — 実行主体ごとの reviewer / self-review 経路で確認
7. **コミット・プッシュと完了報告** — 適切な粒度で commit/push (デフォルトブランチ上なら機能ブランチを自動作成)。PR は作らず別途指示を待つ

7 フェーズは**省略不可の品質ゲート**であって、可変のタスクリストではない。作業タスクの中身はタスクごとに判断してよいが、フェーズの順序と通過は固定。特に **trivial 以外は検証 (5)・レビュー (6) を飛ばさない**。進捗の可視化が要る規模なら、実行主体の harness にある進捗管理 tool を使ってよい。

### 承認済みプランからのエントリ

承認済みプランから実装が始まる場合、プランが本 Skill のフェーズ 3.5 由来かどうかに関わらず:

- **フェーズ 1〜3.5 はスキップ可。** 承認済みプランを作業仕様 (課題 + 受け入れ条件 + 設計判断) として扱う。受け入れ条件が明記されていなければプラン本文から抽出し、仮定を記録する
- **フェーズ 4 以降のゲートはすべて通常通り適用する。** 着手前の担当宣言、検証・レビュー・コミット、trivial 判定基準も通常と同一
...
プラン提示と承認の取り方は `references/execution-actors.md` の **進捗管理とプラン承認** に従う。プランには以下を含める:
...
- **触る可能性のあるファイル glob**
- **検証方針** — 型/ビルド/テスト/lint のうち何を実行するか

プランはコードを書かずに作る。承認が必要なケースでは、承認を得てからフェーズ 4 に進む。拒否 / 修正フィードバック / 承認後の方針変更があれば、フィードバックを反映して再提示する。
...
担当宣言、Claude Code での Codex 委譲、Codex での直接実装、レビュー経路の差分は `references/execution-actors.md` に従う。実装に着手する**前に**担当を 1 行で宣言し、最終報告にも残す。
...
実行主体ごとの比較経路は `references/execution-actors.md` の **視覚比較** に従う。独立 reviewer を使う場合に渡す情報:
...
判定は PASS / NEEDS_REVISION / 判定不能 と差分リストで記録する。Figma 画像が欠けていた場合は PASS にしない。

**視覚反復ループ:** NEEDS_REVISION → must 指摘を反映して実装を修正し、4e (スクショ再取得) → 4f → 4g を再実行。PASS → フェーズ 5 へ。

**ストップ条件:** 4g 反復は最大 3 回。超えても収束しないなら、残差をそのままユーザーに提示して判断を仰ぐ。空回りより escalate を優先。
...
失敗したら修正して再実行する。機械的に潰せる失敗 (型不一致、import 不足、lint 違反、明確な assertion ずれ) は、実行主体ごとの実装経路で修正してよい。失敗が設計の見直し (仕様の取り違え、責務配置の誤り) を示唆する場合はプランへ戻って判断する。

**UI タスクで型エラー修正により実装が変わった場合は、4e (スクショ再取得) → 4f → 4g (視覚比較) を再実行。**

## フェーズ 6: レビュー (trivial 変更を除く)

フェーズ 5 を通過したら、`references/execution-actors.md` の **レビュー** に従って、correctness と style の観点を分けてレビューする。

### trivial 変更のスキップ条件

以下のすべてを満たす変更は **レビューをスキップ**してよい:

- 変更ファイル数が 2 以下、かつ
- 変更行数 (追加 + 削除) が 30 行未満、かつ
- 公開境界 (HTTP API / proto / DB / export 型) に触れていない

または、実行主体が **「レビュー不要」と理由付きで明示宣言**した場合もスキップしてよい (例: typo 修正、コメントのみの変更、設定値の単純差し替え)。宣言は最終報告に 1 行残す。
...
- **correctness** — 正確性、エッジケース、型安全性、セキュリティ、テスト網羅性、フェーズ 1 / 3.5 で置いた仮定
- **style** — 既存パターン整合、最小差分、公開境界不変、命名、類似実装パス

### レビュー結果の処理

レビューの判定をまとめて評価する。指摘は重大度を判定して must / imo 相当に振り分ける:

- **PASS かつ重大指摘なし** → フェーズ 7 へ
...
imo 指摘は採用 / 不採用を判断する。不採用にする場合は理由を最終報告に残す。
...
最終結果、担当宣言 (実行主体 / 委譲有無と理由)、作成したコミット、push 先ブランチ (worktree を作った場合はそのパス) をユーザーに報告する。

## References

実行主体:

- `references/execution-actors.md` — Claude Code / Codex の tool、委譲、レビュー、Skill ディレクトリ解決の差分
