...
7. **完了報告** — UI タスクは side-by-side でユーザー最終確認
...
ローカルチェック通過 (+ 該当時は reviewer PASS) で完了。

**UI タスクの場合**: `/tmp/dev-task-visual-check/<project-basename>/` に残っている Playwright スクショと Figma フレーム画像を side-by-side で見せ、ユーザー最終確認を求める。

最終結果をユーザーに報告。コミット / push / PR は明示的に依頼されない限りユーザー側の操作。
