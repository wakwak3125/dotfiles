...
- [ ] 4: 実装 (タスク性質でメイン / Codex を選択。UI タスクは 4a 抽出 → 4b token-map → 4c 棚卸し → 4d 実装 → 4e 画像ペア → 4f 構造チェック → 4g 視覚比較)
- [ ] 5: 検証 (型 / ビルド / テスト / lint。機械的失敗の修正は Codex 可)
- [ ] 6: レビュー (trivial 除き Claude reviewer 2 体を並列起動 + Codex レビュー併用)
- [ ] 7: コミット・プッシュ・完了報告
```
