...
最初に実行主体を判定し、対応する adapter を読む。以降、ホスト固有の tool / subagent / plan approval / Skill ディレクトリ解決は adapter を優先する。
...
3.5. **プラン作成 (PLAN_REQUIRED のみ)** — runtime adapter の承認手段で plan approval を取り、承認後だけ実装へ進む
...
**まず runtime adapter の plan approval 手順に入る。** Claude Code では `EnterPlanMode` / `ExitPlanMode`、Codex では plan 提示とユーザー承認待ちを使う。これにより承認前に実装へ進む事故を防ぐ。
...
planner / メインはコードを書かず、プランだけを返す。runtime adapter の承認手段で明示的な承認を得てからフェーズ 4 に進む。

- **承認された** → 承認済みプランを workspec に追記し、plan mode が解除されるので、フェーズ 4 へ
- **拒否 / 修正フィードバック / 承認後の方針変更** → フィードバックを反映したプランを再構成し、再度 adapter の承認手段で提示する。planner subagent の再起動は不要 (メインが差分を直接反映してよい)
...
runtime adapter が subagent を提供する場合は、迷ったら委譲する。「設計判断が要る」「既存コードの読み解きが要る」は委譲を避ける理由に**ならない** — 設計判断はフェーズ 1〜3.5 でメインが終えているべきで、読み解いた結果 (参照パターン・対象パス・適用ルール) は委譲仕様に書いて渡せる。それを書き切れないなら、足りないのは委譲可否ではなく上流の整理。
...
- `references/runtime-claude-code.md` — Claude Code での plan mode / Agent / reviewer / Skill ディレクトリ解決
- `references/runtime-codex.md` — Codex での plan approval / main-agent 実装 / self-review / Skill ディレクトリ解決
