---
name: dev-task
description: Linear チケット ID または自然言語で指定された開発タスクを、軽量(最小差分・過剰設計を避ける)かつ高品質(既存パターンに沿い、型/ビルド/テストが通る)に実装する。フロントエンド UI タスク(React TypeScript + Figma 起点)では、Figma MCP で仕様を抽出し Storybook + Playwright で視覚検証ループを回す。「ABC-123 を実装して」「このチケットお願い」「このバグ直して」「〜の対応を追加して」「動くようにして」「このデザインを実装して」「Figma から作って」「このコンポーネント作って」「ピクセルパーフェクトに」など、リポジトリ内のコード変更を伴う依頼で必ず発火する。Kotlin バックエンド、React TypeScript フロントエンド、protobuf スキーマ、Node.js サービスを主対象。ユーザーが「実装」と明言しなくても、具体的なコード変更依頼であれば常に使うこと。
---
...
3.5. **プラン作成 (条件付き)** — PLAN_REQUIRED 時のみ `dev-task-planner` subagent を起動
4. **実装** — *実装ルール*に従う。バックエンド Kotlin の実装は **forge skill で Codex に委譲**。UI タスクの場合は *UI サブフェーズ* (4a〜4g) を実行
5. **検証** — プロジェクトが対応する型/ビルド/テスト/lint を実行
6. **レビュー (条件付き)** — trivial 変更を除き、`dev-task-reviewer-correctness` と `dev-task-reviewer-style` を**並列起動**
7. **コミット・プッシュと完了報告** — 適切な粒度で commit/push (デフォルトブランチ上なら wt で機能ブランチを自動作成)。PR は作らず別途指示を待つ。UI タスクは side-by-side で最終確認

### 進捗チェックリスト

以下のチェックリストを応答にコピーし、フェーズ完了ごとにチェックして進捗を追跡する:

```
Task Progress:
- [ ] 1: 入力解決 (課題 + 受け入れ条件)
- [ ] 2: コンテキスト収集 (類似実装の特定 / UI タスク判定)
- [ ] 3: トリアージ (直接実装 or プラン提示。PLAN_REQUIRED なら 3.5 で承認取得)
- [ ] 4: 実装 (バックエンド Kotlin は forge skill で Codex に委譲。UI タスクは 4a 抽出 → 4b token-map → 4c 棚卸し → 4d 実装 → 4e 画像ペア → 4f 構造チェック → 4g 視覚比較)
- [ ] 5: 検証 (型 / ビルド / テスト / lint)
- [ ] 6: レビュー (trivial 除き reviewer 2 体を並列起動)
- [ ] 7: コミット・プッシュ・完了報告
```
...
`dev-task-planner` subagent を Agent tool で起動する。subagent には以下を構造化して渡す:
...
planner はコードを書かず、プランだけを返す。**得られたプランをユーザーに提示し、明示的な承認を得てから**フェーズ 4 に進む。

承認後にユーザーから方針変更が入った場合は、planner を再度起動するのではなく、メインがその差分を反映したプランを直接提示し直してよい。
...
**影響レイヤーにバックエンド Kotlin が含まれる場合、その実装作業はメインが直接書かず、Skill tool で `forge:forge` を起動して Codex に委譲する。** フロント React-TS / proto / Node.js は従来通りメインが直接実装する。

**例外: 軽微な変更はメインが直接実装してよい。** 目安はフェーズ 6 の trivial スキップ条件と同等 (変更ファイル数 2 以下 / 変更行数 30 行未満 / 公開境界に触れない)、または typo 修正・コメントのみ・設定値の単純差し替えなど。直接実装した場合は「forge 委譲スキップ」と理由を最終報告に 1 行残す。判断に迷ったら委譲する。
...
1. `references/kotlin.md` を読んだ上で、**自己完結したプロンプト**を組み立てる (Codex は会話のコンテキストを一切持たない):
   - 課題と受け入れ条件 (フェーズ 1 の抽出結果)
   - 対象ファイル・関数のパス (フェーズ 2 で特定したもの)
   - 模倣すべき類似実装のパス (フェーズ 2 で見つけた参照パターン)
   - 適用すべき実装ルール — 本フェーズ冒頭の共通ルール (最小差分 / 新抽象禁止 / 公開境界不変 / 型修正) と `references/kotlin.md` の要点
   - PLAN_REQUIRED だった場合は承認済みプランの該当部分
   - 検証コマンド (`./gradlew compileKotlin` 等、プロジェクトのもの)
...
委譲後のフェーズ 5 (検証)・フェーズ 6 (レビュー)・フェーズ 7 (コミット) は通常通りメインが実行する。reviewer のスキップ判定も委譲の有無に関わらず同じ基準を適用する。
...
フェーズ 5 を通過したら、`dev-task-reviewer-correctness` と `dev-task-reviewer-style` を **1 メッセージ内で Agent tool を 2 つ並べて並列起動**する。シリアル起動は禁止 (実時間が無駄になる)。

### trivial 変更のスキップ条件

以下のすべてを満たす変更は **reviewer をスキップ**してよい:

- 変更ファイル数が 2 以下、かつ
- 変更行数 (追加 + 削除) が 30 行未満、かつ
- 公開境界 (HTTP API / proto / DB / export 型) に触れていない

または、メインエージェントが **「reviewer 不要」と理由付きで明示宣言**した場合もスキップしてよい (例: typo 修正、コメントのみの変更、設定値の単純差し替え)。宣言は最終報告に 1 行残す。
...
両 reviewer の判定をまとめて評価する:

- **両方 PASS** → フェーズ 7 へ
- **どちらか NEEDS_REVISION** → must 指摘を反映して修正、フェーズ 5 から再実行
- **再修正後の再レビューは 2 回まで**。2 回目でもまだ NEEDS_REVISION が残るなら、残差をユーザーに提示して判断を仰ぐ (空回りより escalate を優先)

imo 指摘は採用 / 不採用をメインが判断する。不採用にする場合は理由を最終報告に残す。

**UI タスクで reviewer の must 指摘により実装が変わった場合は、フェーズ 4e → 4f → 4g を再実行。**
