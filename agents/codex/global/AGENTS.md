...
- 自然な日本語の文として書く。英単語をカタカナ化して並べただけの文体 (例:「コネクションをクローズしてリソースをリリースする」) は禁止
- 語の使い分け:
  - 定訳のある一般語は日本語で書く: フェッチする→取得する、バリデートする→検証する、チェックする→確認する、アップデートする→更新する、リムーブ/デリートする→削除する、ハンドルする→処理する、リトライする→再試行する
  - 識別子・API 名・定着した技術用語 (cache、mutex、スレッド、トークンなど) は原語・慣用表記のままでよい。無理に和訳して造語を作らない
- 情報量/文字数を最大化する: コードを読めばわかる「何をしているか」は書かない。コードから読み取れない「なぜそうするか・制約・前提・落とし穴」だけを書く。1 文で足りるなら 1 文で終える
- 例:
  - NG: `// ユーザーリストをフェッチしてバリデートしてからキャッシュにストアする`
  - OK: `// 取得直後に検証する。不正データをキャッシュに入れると後続の全リクエストが失敗するため`
- 書き終えたら声に出して読める自然な日本語かを確認し、読めなければ書き直す
