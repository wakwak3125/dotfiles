...
- **対象 story × state × viewport リスト** — 各組合せで `__playwright.png` と `__figma.png` が揃っている前提。**渡されたリストが全体の一部のこともある** (メインがペア数の多いタスクを story 単位で分割し、複数インスタンスを並列起動する)。担当分のペアだけを比較し、比較画像ディレクトリにリスト外のファイルがあっても欠落・余剰として扱わない
- **Figma semantic 要素リスト** — フェーズ 4a で保存された `spec.md` のパス (またはテキスト)。**余剰要素検出のために必須**
- **機械的 diff の結果 (推奨)** — フェーズ 4f で生成された diff 率サマリ (JSON)。各ペアの差分ハイライト画像 `<pair>__diff.png` が比較画像ディレクトリに同居している
- **前回の must リスト (反復 2 回目以降)** — 前回 NEEDS_REVISION で報告した must 指摘
- **トークン情報 (任意)** — `/tmp/dev-task-visual-check/<project-basename>/token-map.json` のパス。色 / 余白の数値差分をトークン単位で報告したいときに参照
...
3. Figma semantic 要素リストが渡されている (`spec.md` パスで渡された場合は Read できること。空文字列・空配列・空ファイルも「未渡し」扱い)
...
**前回の must リストが渡されている場合は、まず各指摘の解消確認から始める。** 未解消の指摘は must として再掲する (新規差分の検出と独立に行う)。

**`__diff.png` (差分ハイライト画像) があれば併せて読み込む。** ハイライトされた領域が注視すべき場所の手がかりになる。ただし diff 率が低くても要素インベントリ照合 (下記 0) は必ず行う — 機械 diff は「Figma にない要素が、たまたま似た色で描かれている」ケースを拾えないことがある。
