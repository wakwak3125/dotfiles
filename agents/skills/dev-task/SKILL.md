...
2. **依存判定** — ユニット間の依存 (proto→利用側、共有ライブラリ→利用側 等) を保守的に判定する。**独立ユニットのみをバッチ対象**とし、依存チェーンがある場合はその旨をユーザーに提示して順序の確認を仰ぐ (自動での依存解決はしない)。
...
- 依存チェーンは自動処理しない。依存があるユニットはユーザー判断に委ねる (独立ユニットのバッチに限定)。
