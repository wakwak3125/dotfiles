...
長時間・多フェーズになりやすい Skill なので、全フェーズで守る: 判断材料が揃ったら動き、確定済みの事実を再導出しない。進捗の主張 (「テスト green」「視覚 PASS」「push 済み」等) は必ずこのセッションの tool 結果で裏取りし、未検証・スキップ・失敗はそのまま書く。手を止めるのは破壊的・不可逆な操作、スコープの実質変更、ユーザーにしか出せない入力のときだけで、他の曖昧さは仮定を記録して進める (空回りが 2〜3 回続いたら escalate)。テキストだけの意思表明でターンを終えず、宣言した作業はそのターン内で実行する。
...
- **複数ユニット (バッチ)** — 独立した作業依頼が 2 件以上 → フェーズ 0 で `dev-task-worker` へ 1 ユニットずつ委譲
...
0. **バッチ検知・ディスパッチ (複数ユニット時のみ)** — 独立した作業依頼が 2 件以上なら各ユニットを `dev-task-worker` へ委譲し、メインは集約・横断整合に専念。単一ユニットはこのフェーズを飛ばす
...
シグナルはあくまで候補。バッチに入る前に、各チケット ID / 依頼が**独立した作業依頼か、参照にすぎないか** (「〜と同じパターンで」「詳細は〜のコメント参照」等) を判定し、参照のみの ID はユニットにしない。**独立した作業依頼が 2 件以上と確認できた場合のみ**フェーズ 0 に入る。
...
1. **ユニット分解** — 各チケット/依頼を 1 ユニットに正規化し、runtime adapter の進捗管理手段 (Claude Code は `TaskCreate`) に登録する。
   - Linear チケットはここで**メインが本文・コメント・受け入れ条件を取得し、ユニットの作業仕様に全文相当を含める**。worker は MCP ツールを持たず再取得できないため、渡した作業仕様が worker にとっての真実になる。
   - **Figma 起点の UI ユニットは worker に委譲しない。** Figma MCP (4a / 4e-2) とコミット前のユーザー最終確認 (フェーズ 7) が worker では実行できないため、バッチから分離し、全 worker の完了後にメインが単一フローで直接処理する。
...
3. **共有ファイル衝突の事前検出** — 各ユニットが触る見込みのファイル集合 (touch-set) を軽量に見積もる (Linear の言及ファイル、runtime adapter が許すなら Explore subagent での read-only スコーピング)。**touch-set が重なるユニット同士は並列にしない**。ただし worker は常にデフォルトブランチ基点で worktree を切るため、順序だけ守っても先行ユニットの変更は後続の worktree に入らず、**直列化だけでは最終マージの衝突は解消しない**。重なるユニットは依存チェーン (手順 2) と同じ扱いにする: 先行ユニットの PR マージ後に後続を流すか、ユーザーに提示して判断を仰ぐ。見積もりは不完全なので、取りこぼしは手順 7 の横断レビューで事後検出する。
4. **共有コンテキストの用意** — 全 worker に渡す共通事項 (リポジトリ規約・命名・スコープ境界・既知の依存) を 1 つにまとめる。パターンのブレを防ぐ。
5. **スケジューリング & 並列ディスパッチ** — 依存 (手順2) と衝突グループ (手順3) の制約を満たす範囲で、**互いに独立なユニットを並列度上限 (既定 4。この手順が既定値の唯一の定義箇所) まで 1 メッセージ内で並列 spawn** する。依存チェーン・衝突グループ内の後続ユニットは、先行 (blocker) のマージ済みゲート (手順 2・3) を満たすまで**起動しない**。上限超過分はキューし、slot が空き次第起動する。各 worker は指定ブランチ名 (`references/git-workflow.md` の規約に従い、Linear チケットは `wakwak3125/<チケットID>-<slug>`、自然言語は `wakwak3125/<slug>`)・作業仕様・共有コンテキストを受け取り、隔離 worktree でフェーズ 1〜7 を完遂してコンパクトなサマリを返す。
...
7. **横断整合レビュー (必須ゲート)** — 全 worker 完了後、サマリの「横断メモ」(触れた公開境界・共有ファイル・新規パターン) を突き合わせ、以下を必ず検査する:
   ...
   問題があれば該当 worker に修正を再委譲する (最大 2 回、収束しなければユーザーに提示)。再委譲は新しい worker の spawn になり、初回の変更は新しい worktree に存在しないため、**「既存の push 済みブランチ (`<ブランチ名>`) に対する修正である」ことと修正すべき指摘内容を明示して渡す** (worker は `git fetch` + `git switch <既存ブランチ>` で再開する。再開手順は worker 定義側)。
8. **完了報告** — 各ユニットの結果 (状態・PR URL・仮定)、横断整合の所見、依存/衝突で保留したユニット (先行のマージ待ち)、バッチから分離した UI ユニットをまとめて報告する。

**このフェーズ自体は品質ゲートを課さない。** 品質ゲート (検証・レビュー) は各 worker 内のフェーズ 5・6 で適用される。メインの責務はディスパッチと整合性の統括。

既知の制約:

- 並列度の既定値は手順 5 が唯一の定義箇所 (他文書は参照のみ)。負荷やサマリ肥大を見て調整してよい。
- 共有ファイル衝突の事前検出 (手順 3) はヒューリスティック (実装前に正確な touch-set は分からない)。取りこぼしは手順 7 の横断レビューで事後検出する。
- 依存・衝突で後続になったユニットは先行のマージ後に流す前提 (blocker ブランチを base にする自動化はスコープ外)。未マージならユーザー判断に委ねる。
...
- **投機的な抽象を導入しない。** タスクが明示的に求めていない「念のため」のレイヤー追加・汎用化は悪臭。差分や重複を減らす局所的なヘルパー (同一ファイル / 同一モジュール内で完結するもの) は最小差分の範囲として可。
