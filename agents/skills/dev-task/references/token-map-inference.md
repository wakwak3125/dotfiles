...
- 前回マップ構築以降、コード側トークン定義が変更されているとき (token-map ファイルの mtime と比較)
- 対象フレームの Figma 変数に既存マップに無い名前が含まれるとき

保存先は `/tmp/dev-task-visual-check/<project-basename>/token-map.json` (`<project-basename>` は CWD のディレクトリ名)。プロジェクト別に分けることで、複数プロジェクトの並行作業時に上書きされない。
...
`/tmp/dev-task-visual-check/<project-basename>/token-map.json` に永続化。
